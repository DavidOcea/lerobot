...
class BackboneEncoderExport(nn.Module):
    ...
    def forward(self, img0, img1, img2, state, force):
        ...
        pos_weight = self.model.encoder_1d_feature_pos_embed.weight
        encoder_in_pos_embed = [pos_weight[0:1]]

        # ── State token ──
        encoder_in_tokens.append(self.model.encoder_robot_state_input_proj(state))
        encoder_in_pos_embed.append(pos_weight[1:2])

        # ── Force token ──
        encoder_in_tokens.append(self.model.encoder_robot_force_input_proj(force))
        encoder_in_pos_embed.append(pos_weight[2:3])
