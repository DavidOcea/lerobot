#!/bin/bash
# 训练 ACT 精简版: chunk_size=20, temporal_ensemble_coeff=0.01
# 目标: 适配 Jetson Orin 部署 (推理 <30ms)
# 对比: 原版 chunk_size=100, n_action_steps=100, 无 temporal ensemble
...
CUDA_VISIBLE_DEVICES=1 python -m lerobot.scripts.train \
    ...
    --save_freq=40000 \
    \
    --output_dir=outputs/train/act_0611_pickup_long_cs20_te001 \
    --job_name=act_0611_pickup_long_cs20_te001 \
    --policy.push_to_hub=false \
    > act_0611_pickup_long_cs20_te001.log 2>&1 &

echo "Training launched. PID: $!"
echo "Monitor: tail -f /root/workspace/dc_dir/lerobot/act_0611_pickup_long_cs20_te001.log"
